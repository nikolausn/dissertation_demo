...
def airbnb_curator_merge(airbnb_pd):

    # In[4]:


    num_records = airbnb_pd.drop_duplicates("id").shape[0]
    num_records
    ...
    # number of records kept score
    dup_ids = airbnb_pd[airbnb_pd.id_flag==0]
    print("score:",dup_ids.shape[0]/num_records)
    ...
    # number of records kept score
    dup_ids = airbnb_pd[airbnb_pd.host_id_flag==0]
    print("score:",dup_ids.shape[0]/num_records)
    ...
    def clean_neighbourhood_flagging(df):
    ...
    def clean_neighbourhood(df):
        ...
        df = clean_neighbourhood_c2(df)
        df = clean_neighbourhood_c1(df)
        df = clean_neighbourhood_flagging(df)
        return df
    ...
    # number of records kept score
    dup_ids = airbnb_pd[airbnb_pd.neighbourhood_flag==0].drop_duplicates("id")
    print("score:",dup_ids.shape[0]/num_records)
    ...
    def clean_latitude_pls(latitude_string):
        ...
        latitude_string = str(latitude_string)
        ...
        if latitude > 90:
            # return it back to latitude_string
            latitude = latitude_string
        return latitude

    def clean_latitude_t1_detection(df):
        # numeric constraint violation, not a number detection
        def check_nan(val):
            try:
                float(val)
                return True
        ...
        df["latitude_flag"] = df.latitude.apply(lambda x:0 if check_nan(x) else 3) 
        return df

    def clean_latitude_c1(df):
        # only for type t1 numeric constraint violation
        df_mask = df.latitude_flag==3
        df.loc[df_mask,"latitude"] = df[df_mask].latitude.apply(lambda x:clean_latitude_pls(x))
        # return back flag to 0 after processing
        df.loc[df_mask,"latitude_flag"] = 0
        return df


    def clean_latitude_flagging(df):
    ...
    #def clean_latitude_t2_detection(df):    
    #    df["latitude_flag"] = df.latitude.apply(lambda x:0 if check_number(x) else 3) 
    #    return df


    def clean_latitude_c2(df):
        df_ori = df
        df_mask = df.latitude_flag==3

        lat_check_pd = df[df.latitude_flag==0]
        #print(lat_check_pd.shape)
        ...
        right_lati_ls=list(lat_true["latitude"].apply(lambda x:float(x)))
        avg_lati=sum(right_lati_ls)/len(right_lati_ls)
    ...
    def clean_latitude(df):
        #raise Exception("not yet have implementation")
        # do something here
        df = clean_latitude_t1_detection(df)
        df = clean_latitude_c1(df)
        #df = clean_latitude_t2_detection(df)
        df = clean_latitude_c3(df)
        df = clean_latitude_flagging(df)
        return df
    ...
    # number of records kept score
    dup_ids = airbnb_pd[airbnb_pd.latitude_flag==0].drop_duplicates("id")
    print("score:",dup_ids.shape[0]/num_records)
    ...
    airbnb_pd[airbnb_pd.latitude_flag==1]
    ...
    def clean_longi_pls(longi_string):
        ...
        longi_string = str(longi_string)
        longi_string = re.sub("[^0-9.\-]", "", longi_string)
        longi = float(longi_string)

        return longi

    def clean_longitude_t1_detection(df):
        # numeric constraint violation, not a number detection
        def check_nan(val):
            try:
                float(val)
        ...
        df["longitude_flag"] = df.longitude.apply(lambda x:0 if check_nan(x) else 3) 
        return df

    def clean_longitude_c1(df):
        # only for type t1 numeric constraint violation
        df_mask = df.longitude_flag==3
        airbnb_pd.loc[df_mask, 'longitude'] = airbnb_pd.loc[df_mask, 'longitude'].apply(lambda x: clean_longi_pls(x))
        # return back flag to 0 after processing
        df.loc[df_mask,"longitude_flag"] = 0
        return df


    def clean_longitude_flagging(df):
    ...
    def clean_longitude_c3(df):
        #raise Exception("not yet have implementation")
        # do something here
        df["longitude"] = df["longitude"].apply(lambda x: x if check_number(x) else adjust_decimal(x))
        df["longitude_flag"] = df["longitude"].apply(lambda x: 2 if pd.isnull(x) else x)
        df["longitude_flag"] = df["longitude"].apply(lambda x: 0 if check_number(x) else 1)
        return df
    ...
    def clean_longitude(df):
        #raise Exception("not yet have implementation")
        # do something here
        df = clean_longitude_t1_detection(df)
        df = clean_longitude_c1(df)
        #df = clean_latitude_t2_detection(df)
        df = clean_longitude_c3(df)
        df = clean_longitude_flagging(df)
        return df
    ...
    # number of records kept score
    dup_ids = airbnb_pd[airbnb_pd.longitude_flag==0].drop_duplicates("id")
    print("score:",dup_ids.shape[0]/num_records)
    ...
    def clean_room_type(df):
        #raise Exception("not yet have implementation")
        # do something here
        df = clean_room_type_c1(df)
        df = clean_room_type_c3(df)
        return df
    ...
    # number of records kept score
    dup_ids = airbnb_pd[airbnb_pd.room_type_flag==0].drop_duplicates("id")
    print("score:",dup_ids.shape[0]/num_records)
    ...
    def clean_min_nights_pls(nights_string):
        nights_string = str(nights_string)
        nights_string = re.sub("[^0-9.]", "", nights_string)
        try:
            nights = int(nights_string)
        except:
            return nights_string
        return nights

    def clean_minimum_nights_c1(df):
        #raise Exception("not yet have implementation")
        # do something here
        #raise Exception("not yet have implementation")
        # do something here
        nights_mask = df['minimum_nights'].apply(lambda x: not check_integer(x))
        df.loc[nights_mask, 'minimum_nights'] = airbnb_pd.loc[nights_mask, 'minimum_nights'].apply(lambda x: clean_min_nights_pls(x))
        # nikolaus change minimum_nights_flag 1 to 0
        df['minimum_nights_flag'] = df['minimum_nights'].apply(lambda x: 0 if check_integer(x) else 1) 
    ...
    def clean_number_of_reviews_c1(df):
        #raise Exception("not yet have implementation")
        # do something here
        reviews_mask = df['number_of_reviews'].apply(lambda x: not check_integer(x,0,999999))
        df.loc[reviews_mask, 'number_of_reviews'] = airbnb_pd.loc[reviews_mask, 'number_of_reviews'].apply(lambda x: clean_min_nights_pls(x))
        df['number_of_reviews_flag'] = df['number_of_reviews'].apply(lambda x: 0 if check_integer(x,0,999999) else 1) 
    ...
    def clean_number_of_reviews(df):
        #raise Exception("not yet have implementation")
        # do something here
        df = clean_number_of_reviews_c1(df)
        return df
